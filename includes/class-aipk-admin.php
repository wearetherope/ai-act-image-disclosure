...
class AIPK_Admin {
	...
	public static function filter_options() {
		return array(
			...
			'unscanned' => __( 'Not scanned yet', 'ai-act-image-disclosure' ),
		);
	}
	...
	public static function column_content( $column_name, $post_id ) {
		...
		if ( ! $record ) {
			echo '<span class="aipk-badge aipk-badge-none">' . esc_html__( 'Not scanned', 'ai-act-image-disclosure' ) . '</span>';
			if ( current_user_can( 'edit_post', $post_id ) ) {
				echo '<div class="aipk-muted"><a href="' . esc_url( self::quick_url( $post_id, 'rescan' ) ) . '">' . esc_html__( 'Scan now', 'ai-act-image-disclosure' ) . '</a></div>';
			}
			return;
		}
		...
		if ( 'hide' === $disclose ) {
			$sub[] = __( 'badge hidden', 'ai-act-image-disclosure' );
		} elseif ( 'show' === $disclose ) {
			$sub[] = __( 'badge forced', 'ai-act-image-disclosure' );
		}
		if ( $sub ) {
	...
	public static function row_actions( $actions, $post ) {
		...
		if ( $record && $record['ai'] ) {
			$actions['aipk_not'] = '<a href="' . esc_url( self::quick_url( $post->ID, 'not_ai' ) ) . '">' . esc_html__( 'Not AI', 'ai-act-image-disclosure' ) . '</a>';
			$hidden              = 'hide' === AIPK_Processor::disclose( $post->ID );
			$actions['aipk_badge'] = '<a href="' . esc_url( self::quick_url( $post->ID, $hidden ? 'badge_default' : 'badge_hide' ) ) . '">' . ( $hidden ? esc_html__( 'Show badge', 'ai-act-image-disclosure' ) : esc_html__( 'Hide badge', 'ai-act-image-disclosure' ) ) . '</a>';
		} else {
			$actions['aipk_ai'] = '<a href="' . esc_url( self::quick_url( $post->ID, 'ai_generated' ) ) . '">' . esc_html__( 'Mark as AI', 'ai-act-image-disclosure' ) . '</a>';
		}
		return $actions;
	}
	...
	public static function bulk_actions( $actions ) {
		$actions['aipk_ai_generated'] = __( 'AI marking: AI generated', 'ai-act-image-disclosure' );
		$actions['aipk_ai_modified']  = __( 'AI marking: AI modified', 'ai-act-image-disclosure' );
		$actions['aipk_not_ai']       = __( 'AI marking: not AI', 'ai-act-image-disclosure' );
		$actions['aipk_auto']         = __( 'AI marking: back to automatic', 'ai-act-image-disclosure' );
		$actions['aipk_rescan']       = __( 'AI marking: re-scan', 'ai-act-image-disclosure' );
		$actions['aipk_show']         = __( 'AI badge: always show', 'ai-act-image-disclosure' );
		$actions['aipk_hide']         = __( 'AI badge: never show', 'ai-act-image-disclosure' );
		$actions['aipk_default']      = __( 'AI badge: follow the settings', 'ai-act-image-disclosure' );
		return $actions;
	}
	...
	public static function panel( $attachment_id ) {
		...
		if ( ! $record ) {
			$html .= '<div class="aipk-panel-head"><span class="aipk-badge aipk-badge-none">' . esc_html__( 'Not scanned yet', 'ai-act-image-disclosure' ) . '</span>';
			if ( $can ) {
				$html .= '<button type="button" class="button button-small aipk-rescan">' . esc_html__( 'Scan file', 'ai-act-image-disclosure' ) . '</button>';
			}
			return $html . '</div></div>';
		}
		...
		if ( $record['suspect'] && $can ) {
			$html .= '<div class="aipk-callout"><p>' . esc_html(
				sprintf(
					...
					__( 'The file carries traces of %s but no formal marking. Confirm to write it.', 'ai-act-image-disclosure' ),
					implode( ', ', $record['signatures'] )
				)
			) . '</p><p><button type="button" class="button button-small button-primary aipk-classify" data-value="ai_generated">' . esc_html__( 'Confirm: AI generated', 'ai-act-image-disclosure' ) . '</button> <button type="button" class="button button-small aipk-classify" data-value="not_ai">' . esc_html__( 'Not AI', 'ai-act-image-disclosure' ) . '</button></p></div>';
		}

		$facts = array(
			__( 'Type', 'ai-act-image-disclosure' )        => $dst ? $dst : __( 'absent', 'ai-act-image-disclosure' ),
			...
			__( 'C2PA', 'ai-act-image-disclosure' )        => $record['has_c2pa'] ? ( $record['generators'] ? implode( ', ', $record['generators'] ) : __( 'present', 'ai-act-image-disclosure' ) ) : '',
			__( 'Traces', 'ai-act-image-disclosure' )      => implode( ', ', $record['signatures'] ),
		...
		if ( $parts ) {
			...
			$html .= esc_html( sprintf( __( 'Sizes marked: %1$d of %2$d', 'ai-act-image-disclosure' ), $ok, count( $parts ) ) );
		} else {
			$html .= esc_html__( 'Generated sizes: nothing to carry', 'ai-act-image-disclosure' );
		}
		$html .= '</summary><ul class="aipk-sizes">' . implode( '', $parts ) . '</ul>';
		$html .= '<p class="aipk-muted">' . esc_html__( 'Blocks in the original:', 'ai-act-image-disclosure' ) . ' ' . esc_html( $blocks ? $blocks : __( 'none', 'ai-act-image-disclosure' ) );
		if ( ! empty( $record['original_marked'] ) ) {
			$html .= ' · ' . esc_html__( 'Original:', 'ai-act-image-disclosure' ) . ' ' . esc_html( self::state_label( $record['original_marked'] ) );
		}
		if ( ! empty( $record['scanned_at'] ) ) {
			$html .= ' · ' . esc_html__( 'Scanned', 'ai-act-image-disclosure' ) . ' ' . esc_html( wp_date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $record['scanned_at'] ) );
		}
		$html .= '</p></details>';
		...
		if ( $can ) {
			...
			$html    .= '<label><span>' . esc_html__( 'Classification', 'ai-act-image-disclosure' ) . '</span><select class="aipk-classify-select">';
			...
			$html .= '</select></label>';
			$html .= '<label><span>' . esc_html__( 'Badge on this image', 'ai-act-image-disclosure' ) . '</span><select class="aipk-disclose-select">';
			...
			$html .= '</select></label>';
			$html .= '<p><button type="button" class="button button-primary button-small aipk-classify-apply">' . esc_html__( 'Apply', 'ai-act-image-disclosure' ) . '</button> <button type="button" class="button button-small aipk-rescan">' . esc_html__( 'Re-scan file', 'ai-act-image-disclosure' ) . '</button></p>';
			$html .= '<p class="aipk-muted">' . esc_html__( 'An AI classification writes the IPTC digital source type into the generated sizes and, unless the original carries a C2PA manifest, into the original too.', 'ai-act-image-disclosure' ) . '</p>';
			$html .= '</div>';
		}
		return $html . '</div>';
	}
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'      => array(
					...
					'reset'    => __( 'Reset every setting to its default?', 'ai-act-image-disclosure' ),
				),
			)
		);
	...
	public static function action_links( $links ) {
		array_unshift( $links, '<a href="' . esc_url( self::url() ) . '">' . esc_html__( 'Settings', 'ai-act-image-disclosure' ) . '</a>' );
		return $links;
	}
	...
	public static function notices() {
		if ( get_transient( 'aipk_activated' ) && current_user_can( 'manage_options' ) ) {
			...
			if ( ! $screen || 'media_page_' . self::PAGE !== $screen->id ) {
				echo '<div class="notice notice-info is-dismissible aipk-notice"><p><strong>' . esc_html__( 'AI Act Image Marking is active.', 'ai-act-image-disclosure' ) . '</strong> ' . esc_html__( 'New uploads are processed automatically. Scan the images you already have and choose how to disclose them.', 'ai-act-image-disclosure' ) . ' <a class="button button-small" href="' . esc_url( self::url() ) . '">' . esc_html__( 'Open settings', 'ai-act-image-disclosure' ) . '</a></p></div>';
			}
		}
		...
		if ( isset( $_GET['aipk_reset'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Settings reset to their defaults.', 'ai-act-image-disclosure' ) . '</p></div>';
		}
	}
	...
	public static function dashboard_widget() {
		...
		wp_add_dashboard_widget( 'aipk_dashboard', __( 'AI images', 'ai-act-image-disclosure' ), array( __CLASS__, 'dashboard_widget_content' ) );
	}
	...
	public static function dashboard_widget_content() {
		...
		self::stat_card( $s['ai'], __( 'AI images', 'ai-act-image-disclosure' ), 'ai', 'ai' );
		self::stat_card( $s['suspect'], __( 'To confirm', 'ai-act-image-disclosure' ), 'suspect', 'suspect' );
		self::stat_card( $s['unscanned'], __( 'Not scanned', 'ai-act-image-disclosure' ), 'unscanned', 'none' );
		echo '</div>';
		$o = AIPK_Options::all();
		echo '<p class="aipk-muted">' . ( $o['badge_enabled'] ? esc_html__( 'Badge on the page: on.', 'ai-act-image-disclosure' ) : esc_html__( 'Badge on the page: off.', 'ai-act-image-disclosure' ) ) . ' <a href="' . esc_url( self::url() ) . '">' . esc_html__( 'Settings', 'ai-act-image-disclosure' ) . '</a></p>';
	}
	...
	public static function environment() {
		...
		$rows[] = array( 'ok', __( 'Image editor', 'ai-act-image-disclosure' ), ( $editor ? $editor : __( 'none', 'ai-act-image-disclosure' ) ) . '. ' . __( 'It strips XMP and IPTC from every generated size; the plugin carries them back after generation.', 'ai-act-image-disclosure' ) );
		...
		$rows[]    = array(
			...
			$threshold
				...
				? sprintf( __( '%d px. Larger uploads are served as a scaled copy, which is marked like every other size.', 'ai-act-image-disclosure' ), (int) $threshold )
				: __( 'Disabled: the original is served as full.', 'ai-act-image-disclosure' ),
		);
		...
		if ( ! empty( $formats ) ) {
			...
			$rows[] = array( $avif ? 'warn' : 'info', __( 'Output format conversion', 'ai-act-image-disclosure' ), implode( ', ', array_map( 'strval', $formats ) ) . ( $avif ? '. ' . __( 'AVIF sizes cannot be written yet and stay unmarked.', 'ai-act-image-disclosure' ) : '' ) );
		}
		...
		$rows[] = $active
			? array( 'warn', __( 'Image optimizers', 'ai-act-image-disclosure' ), implode( ', ', $active ) . '. ' . __( 'They rewrite image files after WordPress and may strip metadata: disable their "strip metadata" option, or re-scan the library after they run.', 'ai-act-image-disclosure' ) )
			: array( 'ok', __( 'Image optimizers', 'ai-act-image-disclosure' ), __( 'None detected.', 'ai-act-image-disclosure' ) );

		$rows[] = function_exists( 'iptcparse' )
			? array( 'ok', __( 'PHP', 'ai-act-image-disclosure' ), PHP_VERSION . ', ' . __( 'IPTC parsing available.', 'ai-act-image-disclosure' ) )
			: array( 'warn', __( 'PHP', 'ai-act-image-disclosure' ), PHP_VERSION . ', ' . __( 'iptcparse() unavailable: IPTC captions are not read (XMP still is).', 'ai-act-image-disclosure' ) );

		$rows[] = wp_is_writable( wp_upload_dir()['basedir'] )
			? array( 'ok', __( 'Uploads folder', 'ai-act-image-disclosure' ), __( 'Writable.', 'ai-act-image-disclosure' ) )
			: array( 'warn', __( 'Uploads folder', 'ai-act-image-disclosure' ), __( 'Not writable: the marking cannot be carried into sizes.', 'ai-act-image-disclosure' ) );
		return $rows;
	}
	...
	public static function settings_page() {
		...
		$tabs      = array(
			'marking'     => __( 'Marking', 'ai-act-image-disclosure' ),
			'disclosure'  => __( 'Disclosure', 'ai-act-image-disclosure' ),
			'environment' => __( 'Environment', 'ai-act-image-disclosure' ) . ( $warns ? ' <span class="aipk-count">' . (int) $warns . '</span>' : '' ),
		);
		?>
		<div class="wrap aipk-settings">
			<div class="aipk-hero">
				<div>
					...
					<p><?php esc_html_e( 'Mark, keep and disclose AI-generated images. Every upload is processed automatically; scan the library once for what came before.', 'ai-act-image-disclosure' ); ?></p>
				</div>
				<div class="aipk-hero-actions">
					<button type="button" class="button button-primary" id="aipk-scan" data-all="0"><?php esc_html_e( 'Scan new images', 'ai-act-image-disclosure' ); ?></button>
					<button type="button" class="button" id="aipk-scan-all" data-all="1"><?php esc_html_e( 'Re-scan everything', 'ai-act-image-disclosure' ); ?></button>
					<a class="button" href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=aipk_export' ), 'aipk_export' ) ); ?>"><?php esc_html_e( 'Export CSV', 'ai-act-image-disclosure' ); ?></a>
				</div>
			</div>
			...
			<div class="aipk-cards">
				<?php
				self::stat_card( $s['images'], __( 'Images in the library', 'ai-act-image-disclosure' ), '', 'plain' );
				self::stat_card( $s['ai'], __( 'AI generated or modified', 'ai-act-image-disclosure' ), 'ai', 'ai' );
				self::stat_card( $s['suspect'], __( 'Suspected AI, to confirm', 'ai-act-image-disclosure' ), 'suspect', 'suspect' );
				self::stat_card( $s['unscanned'], __( 'Not scanned yet', 'ai-act-image-disclosure' ), 'unscanned', 'none' );
				?>
			</div>

			<nav class="nav-tab-wrapper aipk-tabs" aria-label="<?php esc_attr_e( 'Settings sections', 'ai-act-image-disclosure' ); ?>">
			...
			<form method="post" action="options.php" id="aipk-form">
				...
				<section class="aipk-tab" data-tab="marking">
					<div class="aipk-box">
						<h2><?php esc_html_e( 'Uploaded original', 'ai-act-image-disclosure' ); ?></h2>
						<fieldset>
							<label class="aipk-choice"><input type="radio" name="aipk_settings[original_mode]" value="keep" <?php checked( $o['original_mode'], 'keep' ); ?>><span><strong><?php esc_html_e( 'Leave it exactly as uploaded', 'ai-act-image-disclosure' ); ?></strong><em><?php esc_html_e( 'Recommended. The file the client sent is the file you keep.', 'ai-act-image-disclosure' ); ?></em></span></label>
							<label class="aipk-choice"><input type="radio" name="aipk_settings[original_mode]" value="clean" <?php checked( $o['original_mode'], 'clean' ); ?>><span><strong><?php esc_html_e( 'Remove post-production traces, keep the marking', 'ai-act-image-disclosure' ); ?></strong><em><?php esc_html_e( 'Drops Camera Raw settings, document history, ancestors and the creator tool from the XMP. Provenance, description, credits and rights stay. Files with a C2PA manifest are never touched.', 'ai-act-image-disclosure' ); ?></em></span></label>
						</fieldset>
						<label class="aipk-check"><input type="checkbox" name="aipk_settings[manual_writes_original]" value="1" <?php checked( $o['manual_writes_original'] ); ?>> <?php esc_html_e( 'When an image is classified as AI by hand, write the IPTC digital source type into the original too (never on files with a C2PA manifest)', 'ai-act-image-disclosure' ); ?></label>
					</div>
					...
					<div class="aipk-box">
						...
						<div class="aipk-grid-2">
							<fieldset>
								<legend><?php esc_html_e( 'Which images', 'ai-act-image-disclosure' ); ?></legend>
								<label class="aipk-choice"><input type="radio" name="aipk_settings[scope]" value="ai" <?php checked( $o['scope'], 'ai' ); ?>><span><strong><?php esc_html_e( 'AI generated and AI modified only', 'ai-act-image-disclosure' ); ?></strong><em><?php esc_html_e( 'Recommended.', 'ai-act-image-disclosure' ); ?></em></span></label>
								<label class="aipk-choice"><input type="radio" name="aipk_settings[scope]" value="all" <?php checked( $o['scope'], 'all' ); ?>><span><strong><?php esc_html_e( 'Every image with XMP or IPTC metadata', 'ai-act-image-disclosure' ); ?></strong><em><?php esc_html_e( 'Also carries photographer credits and captions, and with them any personal data the metadata may contain.', 'ai-act-image-disclosure' ); ?></em></span></label>
							</fieldset>
							<fieldset>
								<legend><?php esc_html_e( 'What to carry', 'ai-act-image-disclosure' ); ?></legend>
								<label class="aipk-choice"><input type="radio" name="aipk_settings[derivative_mode]" value="full" <?php checked( $o['derivative_mode'], 'full' ); ?>><span><strong><?php esc_html_e( 'The whole XMP and IPTC blocks', 'ai-act-image-disclosure' ); ?></strong><em><?php esc_html_e( 'Recommended.', 'ai-act-image-disclosure' ); ?></em></span></label>
								<label class="aipk-choice"><input type="radio" name="aipk_settings[derivative_mode]" value="minimal" <?php checked( $o['derivative_mode'], 'minimal' ); ?>><span><strong><?php esc_html_e( 'Only the provenance fields', 'ai-act-image-disclosure' ); ?></strong><em><?php esc_html_e( 'Digital source type, description, creator, credit, rights, usage terms. About one kilobyte per file.', 'ai-act-image-disclosure' ); ?></em></span></label>
								<label class="aipk-choice"><input type="radio" name="aipk_settings[derivative_mode]" value="none" <?php checked( $o['derivative_mode'], 'none' ); ?>><span><strong><?php esc_html_e( 'Nothing', 'ai-act-image-disclosure' ); ?></strong><em><?php esc_html_e( 'Sizes stay as WordPress makes them.', 'ai-act-image-disclosure' ); ?></em></span></label>
							</fieldset>
						</div>
						<fieldset class="aipk-sizes-pick">
							<legend><?php esc_html_e( 'Sizes to mark', 'ai-act-image-disclosure' ); ?></legend>
							...
							<?php endforeach; ?>
							<p class="description"><?php esc_html_e( 'Unchecked sizes stay unmarked. Sizes registered by the theme later are marked by default.', 'ai-act-image-disclosure' ); ?></p>
						</fieldset>
					</div>
				</section>
				...
				<section class="aipk-tab" data-tab="disclosure">
					<div class="aipk-grid-2 aipk-grid-sticky">
						<div>
							<div class="aipk-box">
								<h2><?php esc_html_e( 'AI badge on the image', 'ai-act-image-disclosure' ); ?></h2>
								<label class="aipk-check aipk-switch"><input type="checkbox" name="aipk_settings[badge_enabled]" value="1" <?php checked( $o['badge_enabled'] ); ?> data-preview="badge"> <?php esc_html_e( 'Overlay a round, semi-transparent badge on AI generated images', 'ai-act-image-disclosure' ); ?></label>
								<div class="aipk-fields">
									<label><span><?php esc_html_e( 'Text', 'ai-act-image-disclosure' ); ?></span><input type="text" name="aipk_settings[badge_text]" value="<?php echo esc_attr( $o['badge_text'] ); ?>" maxlength="12" data-preview="text"></label>
									<label><span><?php esc_html_e( 'Position', 'ai-act-image-disclosure' ); ?></span><select name="aipk_settings[badge_position]" data-preview="position">
									...
									<label><span><?php esc_html_e( 'Diameter (px)', 'ai-act-image-disclosure' ); ?></span><input type="number" name="aipk_settings[badge_size]" value="<?php echo (int) $o['badge_size']; ?>" min="12" max="120" data-preview="size"></label>
									<label><span><?php esc_html_e( 'Hide under (px wide)', 'ai-act-image-disclosure' ); ?></span><input type="number" name="aipk_settings[badge_min_width]" value="<?php echo (int) $o['badge_min_width']; ?>" min="0"></label>
									<label><span><?php esc_html_e( 'Background', 'ai-act-image-disclosure' ); ?></span><input type="color" name="aipk_settings[badge_bg]" value="<?php echo esc_attr( $o['badge_bg'] ); ?>" data-preview="bg"></label>
									<label><span><?php esc_html_e( 'Opacity (%)', 'ai-act-image-disclosure' ); ?></span><input type="number" name="aipk_settings[badge_opacity]" value="<?php echo (int) $o['badge_opacity']; ?>" min="0" max="100" data-preview="opacity"></label>
									<label><span><?php esc_html_e( 'Text color', 'ai-act-image-disclosure' ); ?></span><input type="color" name="aipk_settings[badge_color]" value="<?php echo esc_attr( $o['badge_color'] ); ?>" data-preview="color"></label>
									<label class="aipk-field-wide"><span><?php esc_html_e( 'Tooltip and screen reader text', 'ai-act-image-disclosure' ); ?></span><input type="text" name="aipk_settings[badge_title]" value="<?php echo esc_attr( $o['badge_title'] ); ?>"></label>
									<div class="aipk-field-wide aipk-image-pick">
										<span><?php esc_html_e( 'Badge image instead of text', 'ai-act-image-disclosure' ); ?></span>
							...
							<div class="aipk-box">
								<h2><?php esc_html_e( 'Popup on the badge', 'ai-act-image-disclosure' ); ?></h2>
								<label class="aipk-check aipk-switch"><input type="checkbox" name="aipk_settings[popup_enabled]" value="1" <?php checked( $o['popup_enabled'] ); ?> data-preview="popup"> <?php esc_html_e( 'A click on the badge opens a panel with the provenance of the image', 'ai-act-image-disclosure' ); ?></label>
								<div class="aipk-fields">
									<label class="aipk-field-wide"><span><?php esc_html_e( 'Panel title', 'ai-act-image-disclosure' ); ?></span><input type="text" name="aipk_settings[popup_title]" value="<?php echo esc_attr( $o['popup_title'] ); ?>" data-preview="popupTitle"></label>
								</div>
								<label class="aipk-check"><input type="checkbox" name="aipk_settings[credit_link]" value="1" <?php checked( $o['credit_link'] ); ?> data-preview="credit"> <?php esc_html_e( 'Show a credit line in the panel: "Marked with AI Act Image Marking by The Rope", linking to therope.it', 'ai-act-image-disclosure' ); ?></label>
								<p class="description"><?php esc_html_e( 'The panel is plain HTML kept hidden until the badge is clicked, readable by search engines and assistive technology. The credit line is your choice and is off by default.', 'ai-act-image-disclosure' ); ?></p>
							</div>

							<div class="aipk-box">
								<h2><?php esc_html_e( 'Text, notice, data', 'ai-act-image-disclosure' ); ?></h2>
								<label class="aipk-check aipk-switch"><input type="checkbox" name="aipk_settings[frontend_label]" value="1" <?php checked( $o['frontend_label'] ); ?> data-preview="label"> <?php esc_html_e( 'Print a short text under AI generated images', 'ai-act-image-disclosure' ); ?></label>
								<div class="aipk-fields"><label class="aipk-field-wide"><span><?php esc_html_e( 'Text', 'ai-act-image-disclosure' ); ?></span><input type="text" name="aipk_settings[label_text]" value="<?php echo esc_attr( $o['label_text'] ); ?>" data-preview="labelText"></label></div>
								<p class="description"><?php esc_html_e( 'Also used as disclosure for images classified by hand that carry none.', 'ai-act-image-disclosure' ); ?></p>
								<label class="aipk-check aipk-switch"><input type="checkbox" name="aipk_settings[footer_notice]" value="1" <?php checked( $o['footer_notice'] ); ?>> <?php esc_html_e( 'Print a one-line transparency notice at the end of every page', 'ai-act-image-disclosure' ); ?></label>
								<div class="aipk-fields"><label class="aipk-field-wide"><span><?php esc_html_e( 'Notice', 'ai-act-image-disclosure' ); ?></span><input type="text" name="aipk_settings[notice_text]" value="<?php echo esc_attr( $o['notice_text'] ); ?>"></label></div>
								<p class="description"><?php esc_html_e( 'The same text is the default of the [ai_act_disclosure] shortcode and the "AI disclosure notice" block, which also show the number of AI images in the library.', 'ai-act-image-disclosure' ); ?></p>
								<label class="aipk-check"><input type="checkbox" name="aipk_settings[frontend_attrs]" value="1" <?php checked( $o['frontend_attrs'] ); ?>> <?php esc_html_e( 'Add data-digital-source-type and data-ai-generated to image tags', 'ai-act-image-disclosure' ); ?></label>
								<label class="aipk-check"><input type="checkbox" name="aipk_settings[schema_enabled]" value="1" <?php checked( $o['schema_enabled'] ); ?>> <?php esc_html_e( 'Print schema.org ImageObject data with digitalSourceType for the AI images on the page', 'ai-act-image-disclosure' ); ?></label>
							</div>

							<div class="aipk-box">
								<h2><?php esc_html_e( 'Custom CSS', 'ai-act-image-disclosure' ); ?></h2>
						...
						<aside class="aipk-preview-col">
							<div class="aipk-box aipk-preview-box">
								<h2><?php esc_html_e( 'Live preview', 'ai-act-image-disclosure' ); ?></h2>
								<div class="aipk-preview" id="aipk-preview">
									<div class="aipk-preview-stage">
										...
										<div class="aipk-popup" id="aipk-preview-popup" hidden>
											<div class="aipk-popup-head"><span class="aipk-popup-title"><?php echo esc_html( $o['popup_title'] ); ?></span><button type="button" class="aipk-popup-close" aria-label="<?php esc_attr_e( 'Close', 'ai-act-image-disclosure' ); ?>">&times;</button></div>
											<dl class="aipk-popup-rows">
												<dt><?php esc_html_e( 'Type', 'ai-act-image-disclosure' ); ?></dt><dd><?php esc_html_e( 'AI generated', 'ai-act-image-disclosure' ); ?></dd>
												<dt><?php esc_html_e( 'Disclosure', 'ai-act-image-disclosure' ); ?></dt><dd class="aipk-preview-disclosure"><?php echo esc_html( $o['label_text'] ); ?></dd>
												<dt><?php esc_html_e( 'Marking', 'ai-act-image-disclosure' ); ?></dt><dd>IPTC trainedAlgorithmicMedia</dd>
											</dl>
											<p class="aipk-popup-note"><?php esc_html_e( 'Marked under EU Regulation 2024/1689 (AI Act), article 50.', 'ai-act-image-disclosure' ); ?></p>
								...
								<p class="description"><?php esc_html_e( 'Updates as you type. Click the badge to see the popup.', 'ai-act-image-disclosure' ); ?></p>
				...
				<section class="aipk-tab" data-tab="environment">
					...
					<div class="aipk-box">
						<h2><?php esc_html_e( 'Tools', 'ai-act-image-disclosure' ); ?></h2>
						<p><?php esc_html_e( 'Shortcode:', 'ai-act-image-disclosure' ); ?> <code>[ai_act_disclosure]</code> · <?php esc_html_e( 'Block:', 'ai-act-image-disclosure' ); ?> <?php esc_html_e( 'AI disclosure notice', 'ai-act-image-disclosure' ); ?> · WP-CLI: <code>wp ai-provenance scan</code> · REST: <code>_aipk_provenance</code></p>
						<p><a class="button-link aipk-danger" id="aipk-reset" href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=aipk_reset' ), 'aipk_reset' ) ); ?>"><?php esc_html_e( 'Reset all settings to defaults', 'ai-act-image-disclosure' ); ?></a></p>
					</div>
				</section>

				<div class="aipk-footer"><?php submit_button( null, 'primary', 'submit', false ); ?> <span class="aipk-muted"><?php esc_html_e( 'Settings apply to every page immediately. Files already marked stay marked.', 'ai-act-image-disclosure' ); ?></span></div>
			</form>
		</div>
		<?php
	}
